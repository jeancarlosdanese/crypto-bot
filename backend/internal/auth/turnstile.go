...
func VerifyTurnstile(token string) bool {
	...
	if secretKey == "" {
		logger.Error("TURNSTILE_SECRET_KEY não está configurada.", nil)
		return false
	}
	...
	if err != nil {
		logger.Error("Erro ao fazer requisição para o Turnstile:", err)
		return false
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		logger.Error("Erro ao decodificar resposta do Turnstile:", err)
		return false
	}
