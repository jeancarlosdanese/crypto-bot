// internal/db/postgres/account_otp_repo.go
...
func NewAccountOTPRepository(db *pgxpool.Pool) repository.AccountOTPRepository {
...
func (r *AccountOTPRepoPostgres) FindValidOTP(ctx context.Context, identifier string, otp string) (*uuid.UUID, error) {
	...
	`
	err := r.db.QueryRow(ctx, query, otp, identifier).Scan(&accountID, &expiresAt)
	if err != nil {
...
func (r *AccountOTPRepoPostgres) CleanExpiredOTPs(ctx context.Context) error {
	_, err := r.db.Exec(ctx, "DELETE FROM account_otps WHERE expires_at < now()")
	return err
}

// FindByEmailOrWhatsApp busca uma conta pelo e-mail ou WhatsApp
func (r *AccountOTPRepoPostgres) FindByEmailOrWhatsApp(ctx context.Context, identifier string) (*entity.Account, error) {
...
func (r *AccountOTPRepoPostgres) StoreOTP(ctx context.Context, accountID string, otp string) error {
	...
	_, err := r.db.Exec(ctx, query, accountID, otp, expiration)
	return err
}
...
func (r *AccountOTPRepoPostgres) GetOTPAttempts(ctx context.Context, identifier string) (int, error) {
	...
	query := `
        ...
        LIMIT 1
    `
	err := r.db.QueryRow(ctx, query, identifier).Scan(&attempts)
	if err != nil {
		if err == sql.ErrNoRows {
