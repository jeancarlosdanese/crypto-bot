...
func (r *BotRepository) GetByID(id uuid.UUID) (*entity.BotWithStrategy, error) {
	...
	SELECT 
		...
		b.created_at, b.updated_at, s.id AS strategy_id, s.name AS strategy_name
	FROM bots b
...
`
	...
	err := row.Scan(
		...
		&b.CreatedAt, &b.UpdatedAt, &b.StrategyID, &b.StrategyName,
	)
	if err != nil {
		return nil, err
	}
...
func (r *BotRepository) Update(bot *entity.Bot) (*entity.BotWithStrategy, error) {
	...
	_, err := r.db.Exec(context.Background(), query,
		...
		bot.Autonomous, bot.Active, bot.ID,
	)
	if err != nil {
		return nil, err
	}
